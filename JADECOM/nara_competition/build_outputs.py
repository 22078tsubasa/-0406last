...
def read_matrix_excel(path: Path, shp_path: Path) -> tuple[pd.DataFrame, list[str]]:
    df = pd.read_excel(path)
    if not {"CITY_NAME_from_master", "S_NAME_from_master", "town_name_show"}.issubset(df.columns):
        raise RuntimeError(f"matrix excel columns missing: {path}")

    shp_lookup = build_shp_lookup(shp_path)
    work = df.copy()
    work["_city_"] = normalize_name(work["CITY_NAME_from_master"])
    work["_sname_"] = normalize_name(work["S_NAME_from_master"])
    work = work.merge(shp_lookup, on=["_city_", "_sname_"], how="left")
    work["TOWN_CODE_11"] = work["TOWN_CODE_11"].fillna("").astype(str)
    work = work[work["TOWN_CODE_11"].ne("")].copy()

    skip_cols = META_COLS | {"_city_", "_sname_", "TOWN_CODE_11"}
    facility_cols = [
        c
        for c in work.columns
        if c not in skip_cols and not re.fullmatch(r"total_\d+", str(c))
    ]
    if not facility_cols:
        raise RuntimeError(f"facility columns not found in matrix excel: {path}")

    for col in facility_cols:
        work[col] = pd.to_numeric(work[col], errors="coerce").fillna(0)

    grouped = (
        work[["TOWN_CODE_11", "town_name_show"] + facility_cols]
        .groupby(["TOWN_CODE_11", "town_name_show"], as_index=False)
        .sum()
    )
    return grouped, facility_cols
...
def load_facility_counts(pref_cfg: dict, facility_cfg: dict) -> tuple[pd.DataFrame, list[str]]:
    source_path = facility_cfg.get("source_path")
    if source_path:
        source = Path(source_path)
        if source.suffix.lower() in {".xlsx", ".xls"}:
            return read_matrix_excel(source, pref_cfg["shp_path"])
        raise RuntimeError(f"unsupported source_path: {source}")

    group_dir = pref_cfg["base_data_dir"] / facility_cfg["folder_name"]
    return collect_facility_frames(group_dir)
...
def build_matrix(pref_cfg: dict, facility_cfg: dict) -> tuple[pd.DataFrame, list[str]]:
    counts, facility_cols = load_facility_counts(pref_cfg, facility_cfg)
...
def export_for_facility(pref_key: str, pref_cfg: dict, facility_cfg: dict) -> dict:
    ...
    manifest = {
        ...
        "csvs": {
            ...
            "町丁目別医療機関利用CSV": str(base_matrix_path),
