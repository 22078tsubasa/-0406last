...
BASE_DIR = Path("/Users/itotsubasa/Downloads/jadecom/大阪赤十字病院/勢力図分析/一部抜粋")

IMAGE_FILES = {
    "全体勢力図": BASE_DIR / "dominant_territory_osaka_car30_usersTop60_一部抜粋_複合施設除外版.png",
    "大阪赤十字病院+上位5勢力図": BASE_DIR / "dominant_territory_osaka_car30_osakaPlus5_other_一部抜粋_複合施設除外版.png",
    "上位10ヒートマップ図": BASE_DIR / "osaka_car30_usersTop60_town_med_heatmap_上位10_一部抜粋_複合施設除外版.png",
    "上位3マトリクス図": BASE_DIR / "osaka_car30_usersTop60_town_med_matrix_上位3_一部抜粋_複合施設除外版.png",
    "グラフ": BASE_DIR / "town_med_graph_top60.png",
}

CSV_FILES = {
    "全体勢力図データ": BASE_DIR / "dominant_territory_osaka_car30_usersTop60_一部抜粋_複合施設除外版_result.csv",
    "大阪赤十字+上位5勢力図データ": BASE_DIR / "dominant_territory_osaka_car30_osakaPlus5_other_一部抜粋_複合施設除外版_result.csv",
    "上位3マトリクスCSV": BASE_DIR / "osaka_car30_usersTop60_town_med_matrix_上位3のみ_一部抜粋_複合施設除外版.csv",
    "上位10マトリクスCSV": BASE_DIR / "osaka_car30_usersTop60_town_med_matrix_上位10_一部抜粋_複合施設除外版.csv",
    "施設ランキングCSV": BASE_DIR / "osaka_car30_usersTop60_facility_rank_summary_一部抜粋_複合施設除外版.csv",
    "選定施設CSV": BASE_DIR / "osaka_car30_usersTop60_selected_facilities_一部抜粋_複合施設除外版.csv",
}
...
def render_header() -> None:
    st.markdown(
        """
        <div class="hero-card">
          <div class="hero-title">大阪赤十字病院競合分析（複合施設除外版）</div>
...
def render_map_tab() -> None:
    ...
    with right:
        render_zoomable_image("大阪赤十字病院+上位5勢力図", IMAGE_FILES["大阪赤十字病院+上位5勢力図"], "map_plus5")
...
def render_data_tab() -> None:
    ...
    plus5_df = read_csv_safely(CSV_FILES["大阪赤十字+上位5勢力図データ"])
    dom_col = None
    ...
    if not plus5_df.empty and dom_col is not None:
        st.subheader("大阪赤十字+上位5の勢力分布（町丁目数）")
...
def render_download_tab() -> None:
    ...
    st.download_button(
        ...
        data=zip_bytes,
        file_name="osaka_partial_report_bundle.zip",
        mime="application/zip",
    )


st.set_page_config(page_title="大阪赤十字病院競合分析（一部抜粋）", layout="wide")
require_password()
inject_style()
render_header()
...
st.info(
    ...
    "./.venv/bin/streamlit run JADECOM/大阪年齢別需要者数/streamlit_report_osaka_partial.py`"
)
