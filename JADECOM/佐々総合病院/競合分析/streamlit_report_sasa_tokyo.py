...
BASE_DIR = Path(
    "/Users/itotsubasa/Downloads/jadecom/東京佐々総合病院/競合分析柘植と同じ形/修正後"
)
...
def render_data_tab() -> None:
    ...
    if not plus5_df.empty and dom_col is not None:
        st.subheader("佐々+上位5の勢力分布（町丁目数）")
        plus5_view = (
            plus5_df[dom_col].fillna("未結合").value_counts().rename_axis("施設名").reset_index(name="町丁目数")
        )
...
st.info(
    ...
    "./.venv/bin/streamlit run JADECOM/佐々総合病院/競合分析/streamlit_report_sasa_tokyo.py`"
)
