...
DOWNLOAD_OUT_DIR = Path("/Users/itotsubasa/Downloads/jadecom/東京佐々総合病院/競合分析柘植と同じ形/修正後")
...
OUT_MATRIX_TOP3_PNG = DOWNLOAD_OUT_DIR / "sasa_car30_usersTop60_town_med_matrix_上位3_町丁目60.png"
OUT_MATRIX_TOP3_CSV = DOWNLOAD_OUT_DIR / "sasa_car30_usersTop60_town_med_matrix_上位3のみ_町丁目60.csv"
OUT_HEATMAP_TOP10_PNG = DOWNLOAD_OUT_DIR / "sasa_car30_usersTop60_town_med_heatmap_上位10_町丁目60.png"
OUT_HEATMAP_TOP10_CSV = DOWNLOAD_OUT_DIR / "sasa_car30_usersTop60_town_med_matrix_上位10_町丁目60.csv"
...
OUT_PLUS5_SELECTION_CSV = DOWNLOAD_OUT_DIR / "sasa_car30_sasaPlus5_selected_facilities.csv"
...
def choose_target_plus_top5(df_top60: pd.DataFrame, top60: list[str]) -> list[str]:
    target = next((c for c in top60 if TARGET_FACILITY_KEYWORD in str(c)), None)
    if target is None:
        raise ValueError(f"{TARGET_FACILITY_KEYWORD} を含む施設列が見つかりません。")

    totals = df_top60[top60].sum(axis=0).sort_values(ascending=False)
    candidates = [c for c in totals.index.tolist() if c != target]
    if PLUS5_HOSPITAL_ONLY:
        hospital_candidates = [c for c in candidates if "病院" in str(c)]
        if len(hospital_candidates) >= TOP_K_EXCLUDING_TARGET:
            candidates = hospital_candidates
    top_ex_target = candidates[:TOP_K_EXCLUDING_TARGET]
    selected = [target] + top_ex_target

    out = pd.DataFrame(
        {
            "rank_excluding_target": np.arange(1, len(top_ex_target) + 1),
            "facility": top_ex_target,
            "total_users": [totals[f] for f in top_ex_target],
        }
    )
    out.insert(0, "target_facility", target)
    out.insert(1, "hospital_only_filter", int(PLUS5_HOSPITAL_ONLY))
    out.to_csv(OUT_PLUS5_SELECTION_CSV, index=False, encoding="utf-8-sig")
    return selected
...
def plot_territory_sasa_plus5_other(df_top60: pd.DataFrame, top60: list[str]) -> None:
    # 佐々総合病院 + 佐々総合病院を除く利用者数上位5施設
    selected = choose_target_plus_top5(df_top60, top60)
...
def plot_top10_heatmap(df_half: pd.DataFrame, facility_cols: list[str], id_cols_existing: list[str], towns: list[str]) -> None:
    ...
    ax.set_yticklabels(towns, fontsize=8)
    ax.set_title("佐々総合病院 車30分圏内: ヒートマップ（利用者数TOP60の中の上位10・上位60町丁目）", fontsize=14)
...
def main() -> None:
    ...
    print("saved:", OUT_PLUS5_SELECTION_CSV)
