...
def build_town_code_11_from_shp(df: pd.DataFrame) -> pd.Series:
    pref = df["PREF"].fillna("").astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(2)
    key1 = df["KEYCODE1"].fillna("").astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(9)
    return pref + key1
...
def read_town_csv(path: Path) -> pd.DataFrame:
    ...
    out = out[out["TOWN_CODE_11"] != "99999999999"].copy()
...
def collect_facility_frames(group_dir: Path) -> tuple[pd.DataFrame, list[str]]:
    ...
    for child in sorted(group_dir.iterdir(), key=lambda p: p.name):
        if not child.is_dir():
            continue
        if child.name.startswith("_") or child.name in {"test"}:
            continue
        town_files = sorted(child.glob("*3_Towns_*.csv"))
        if not town_files:
            continue
        facility = clean_facility_name(child.name)
        if not facility:
            continue
...
def load_demand_master(path: Path) -> pd.DataFrame:
    ...
    return df.drop_duplicates("TOWN_CODE_11", keep="first").reset_index(drop=True)


def build_shp_lookup(shp_path: Path) -> pd.DataFrame:
    shp = gpd.read_file(shp_path)[["PREF", "KEYCODE1", "CITY_NAME", "S_NAME"]].copy()
    shp["TOWN_CODE_11"] = build_town_code_11_from_shp(shp)
...
def build_matrix(pref_cfg: dict, facility_cfg: dict) -> tuple[pd.DataFrame, list[str]]:
    group_dir = pref_cfg["base_data_dir"]
    counts, facility_cols = collect_facility_frames(group_dir)
    demand = load_demand_master(pref_cfg["demand_csv"])

    shp = gpd.read_file(pref_cfg["shp_path"])[["PREF", "KEYCODE1", "CITY_NAME", "S_NAME", "geometry"]].copy()
    shp["TOWN_CODE_11"] = build_town_code_11_from_shp(shp)
    target_codes = load_car30_town_codes(
        facility_cfg["car30_town_csv"],
        pref_cfg["shp_path"],
        facility_cfg["car30_name"],
        pref_cfg["label"],
    )
    shp = shp[shp["TOWN_CODE_11"].isin(target_codes)].copy()
    ...
    merged = base.merge(
        demand.drop(columns=["CITY_NAME_from_master", "S_NAME_from_master", "town_name_show", "LON", "LAT", "dist_km"], errors="ignore"),
        on="TOWN_CODE_11",
        how="left",
    )
    ...
    merged = merged.merge(counts.drop(columns=["town_name_show"], errors="ignore"), on="TOWN_CODE_11", how="left")
    ...
    merged["match_type"] = "code_in_target_towns"
...
def build_geo_join(df: pd.DataFrame, shp_path: Path, value_col: str, max_col: str) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    ...
    gdf["TOWN_CODE_11"] = build_town_code_11_from_shp(gdf)

    work = df.copy()
    work["TOWN_CODE_11"] = work["TOWN_CODE_11"].fillna("").astype(str).str.zfill(11)
    selected_cols = [c for c in df.columns if c not in META_COLS and not c.endswith("_受容者(小数)") and not c.endswith("_受容者(四捨五入)")]
    ...
    hit_cities = sorted(gdfm["CITY_NAME"].dropna().unique().tolist())
    city_outline = gdf[gdf["CITY_NAME"].isin(hit_cities)].dissolve(by="CITY_NAME", as_index=False)
...
def draw_territory(
    ...
    highlight_mask = gdfm[plot_col].astype(str).str.contains(target_label, na=False)
    if highlight_mask.any():
        gdfm[highlight_mask].boundary.plot(ax=ax, color="white", linewidth=3.0, zorder=19)
        gdfm[highlight_mask].boundary.plot(ax=ax, color="red", linewidth=2.0, zorder=20)
    ...
    ax.legend(handles=handles, loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=fs, ncol=ncol, frameon=True)
    ax.set_title(title)
    ax.axis("off")
    ax.set_aspect("equal")
...
def render_top10_heatmap(df_part: pd.DataFrame, facility_cols: list[str], out_png: Path, out_csv: Path, target_keyword: str) -> None:
    ...
    pd.concat([df_part[["town_name_show", "dist_km"]], pd.DataFrame(mat, columns=top)], axis=1).to_csv(out_csv, index=False, encoding="utf-8-sig")
    ...
    labels = [f"{t} ({d:.1f}km)" if pd.notna(d) else t for t, d in zip(df_part["town_name_show"].astype(str), df_part["dist_km"])]
...
def render_top3_matrix(df_part: pd.DataFrame, facility_cols: list[str], out_png: Path, out_csv: Path) -> None:
    ...
    pd.concat([df_part[["town_name_show", "dist_km"]], pd.DataFrame(values_top3, columns=facility_cols)], axis=1).to_csv(out_csv, index=False, encoding="utf-8-sig")
...
def render_graph(df: pd.DataFrame, facility_cols: list[str], out_png: Path, title: str) -> None:
    ...
    labels = [f"{t} ({d:.1f}km)" if pd.notna(d) else t for t, d in zip(df60["town_name_show"].astype(str), df60["dist_km"])]
...
def export_for_facility(pref_key: str, pref_cfg: dict, facility_cfg: dict) -> dict:
    ...
    matrix_jp_path = out_dir / f"{facility_cfg['label']}周辺の町丁目_各医療施設利用数_KLA統合.csv"
    ...
    matrix.to_csv(matrix_jp_path, index=False, encoding="utf-8-sig")
    ...
    rank = pd.DataFrame({"facility": facility_total_part.index, "total_patients": facility_total_part.values, "top3_frequency": top3_frequency.reindex(facility_total_part.index).fillna(0).values})
    ...
    gdf_plus5[["CITY_NAME", "S_NAME", "dominant_sel6", "max_count_sel6", "dist_km", "plot_label"]].to_csv(map_plus5_csv, index=False, encoding="utf-8-sig")
    ...
    manifest = {
        ...
        "csvs": {
            ...
            "町丁目×医療機関統合CSV_日本語名": str(matrix_jp_path),
        },
        "town_count": int(len(matrix)),
    }
...
def main() -> None:
    ...
    for pref_key, pref_cfg in PREFECTURES.items():
        ...
        for facility_cfg in pref_cfg["facilities"]:
            ...
            registry[pref_key]["facilities"].append({"key": facility_cfg["key"], "label": facility_cfg["label"], "manifest": str(OUT_ROOT / pref_key / facility_cfg["key"] / "manifest.json")})
