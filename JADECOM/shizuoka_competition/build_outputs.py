...
def load_car30_town_codes(car30_csv: Path, shp_path: Path, facility_name: str) -> set[str]:
    ...
    pref_mask = df.get("PREF_NAME", pd.Series("", index=df.index)).astype(str).eq("静岡県")
...
def render_top10_heatmap(
    ...
    im = ax.imshow(
        mat_for_color,
        aspect="auto",
        norm=Normalize(vmin=0, vmax=float(np.nanmax(mat_for_color)) if mat_for_color.size else 1),
    )
