...
def load_car30_town_codes(car30_csv: Path, shp_path: Path, facility_name: str, pref_name: str) -> set[str]:
    ...
    raw_key = picked["KEYCODE1"].fillna("").astype(str).str.replace(r"\.0$", "", regex=True)
    pref_code = picked.get("PREF", pd.Series("", index=picked.index)).fillna("").astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(2)
    picked["TOWN_CODE_11"] = np.where(
        raw_key.str.len().eq(9) & pref_code.ne("00"),
        pref_code + raw_key,
        raw_key.map(normalize_key_code),
    )
    direct = {c for c in picked["TOWN_CODE_11"].fillna("").astype(str).tolist() if c and c != "00000000000"}
    if direct:
        return direct
